...
test=0#we increment this number gradually and test to see if it is prime
	...
	while (div<=test): #if you put while True, foe runs through it the first time fine.. but since the while itself doesnt have a condition that will become false at some point in time, it keeps getting executed. Each time it runs, div increases by 1. Now, when div is increasing, no where in the loop, we have any check to trip it if it increases beyond test. If div>test, the if loop will never hold, and with no check on div, it will simply keep increasing, with the loop never ending. So, with while true, you either put a check like if(div>=test): break.. or put this in the condition itself. 
		...
		if (test%div)==0: #instead of counting the number of factors each number has, here we simply break the loop, the first time we find a factor other than one and the number itself.. this way we are discarding any numbers that have a factor other than one, BEFORE reaching the number itself.
			if (div==1)or(div==test):
				...
				break
		div=div+1
		if(div>test/2)&(div!=(test+1)): #this skips testing the other half of numbers upto the test number, and skips directly to the test number. if you just wrote if (div>(test/2)): div=test, the program doesn't print any result, neither does it stop. This is because, the first time this line is executed, div is set equal to test. The loop then executes because the condition to check at line 27 is div<=test (you can't say div<test, because a prime number is div by itself.. and by doing this div will never reach test, so no number will have two divisors.. the program will go on and on, and never stop.. since a will never be 2, and count will never increase and reach n, and thus the outer while loop will never break), then as the loop executes, div becomes test+1 and then this condition is again satisfied, since test plus one is obviously more than test/2.. div is again RESET to test, and the loop repeats endlessly, because on the next iteration div is again test+1.. and the if condition executes again, on and on. To halt it, we have to stop the "if" on executing as and when div is test+1 (because in the endless case it never rises beyond test+1, after which it is reset).. so add the (div!=(test+1)).. since on the next iteration, as div is test+1, the loop is broken, as break executes.. and then a new number is tried.. since foe shifts to line 23. div>=(test/2) is wrong, because when it tries 4, it will make this if statement true, since div is 2 and not 5, and set div equal to 4, and that will make a=2 on the next check, making 4 the third prime number. This makes all future prime numbers off by 1. which means if you search for the 10,000th prime you'll be returned the result of the 9999th prime which is 104723. Beyond the number 4, it doesn't create issues however.
...
# 10000		04:52		249044203		104729		The 9999th prime is 104723 & the 10,000 th prime is 104729 differing only by 6. the 9999th prime takes 248981827 computing cycles.
# 100000
# 500000	
# 1000000